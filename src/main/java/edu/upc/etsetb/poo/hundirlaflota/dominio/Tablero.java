...
public class Tablero {
     ...
     */
    public Tablero() {
       this.casillas= new HashMap<String,Barco>();
       this.disparosAAgua= new ArrayList<String>();
    }
    ...
    /**
     * Intenta poner u
     * n barco en una posición y con una dirección indicados en
     ...
     */
    public void ponBarco(Barco barco, String posicion, String direccion)throws PositionException {
        if(!Posicion.esCorrecta(posicion)){
            throw new PositionException("Posicion Incorrecta");
        }
        Posicion.checkNoContactaConOtro(posicion,barco.getLon(),direccion,this); // para saber si contacta 
        List<String> posiciones = new ArrayList<>();
        posiciones.add(posicion);
        for(int i=1;i<barco.getLon();i++){
            if(direccion.equals("H"))
            posiciones.add(Posicion.avanzaCasillas(posicion, 0, i));
            else posiciones.add(Posicion.avanzaCasillas(posicion,i,0));
        }
    barco.setPosiciones(posiciones);
    for (String p : posiciones) {
        casillas.put(p, barco);
    }
}
