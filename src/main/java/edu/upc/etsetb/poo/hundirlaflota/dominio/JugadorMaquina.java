...
public class JugadorMaquina extends Jugador {
    ...
    public JugadorMaquina(){
        ...
        myRandom = new Random(JugadorMaquina.numInstances); 
        this.nombre = "Maquina-" + JugadorMaquina.numInstances; 
    }
    ...
    String resultado;
        if(this.tocadas.size()==0 ){
            int disparo = this.myRandom.nextInt(this.noDisparadas.size());
            resultado = this.noDisparadas.get(disparo);
            this.noDisparadas.remove(disparo);
            
        } else{
            resultado = this.proximosDisparos.get(0);
            this.proximosDisparos.remove(0);
        }
        return resultado;
}
    ...
    public boolean eresHumano() {
      return false;
        ...
        if(!Posicion.esCorrecta(posDisparo)){
            throw new PositionException("error de posicion");
        }   
        List<String> col = new ArrayList<>(); 
       if (this.dirBarcoTocado.equals("V")) {
        int a=Integer.parseInt(posDisparo.substring(1));
        for(String columnas: proximosDisparos){
            if(Integer.parseInt(columnas.substring(1))!=a){
                col.add(columnas);
            }     
        }
         Listas.quitaAUnaOtra(proximosDisparos, col);
        for (String posicion : Posicion.getNorteSur(posDisparo)) {
            if (!proximosDisparos.contains(posicion) && !tocadas.contains(posicion)) {
                proximosDisparos.add(posicion);
            }
        }
        
    } else {      
        List<String> fil = new ArrayList<>();
        char a=posDisparo.charAt(0);
        for(String filas: proximosDisparos){
            if(filas.charAt(0)!=a){
                fil.add(filas);
            }
        }
        Listas.quitaAUnaOtra(proximosDisparos, fil);
        for (String posicion : Posicion.getEsteOeste(posDisparo)) {
            if (!proximosDisparos.contains(posicion) && !tocadas.contains(posicion)) {
                proximosDisparos.add(posicion);
            }
        }
    ...
    Set<String> P_adyacentes = Posicion.getAdyacentes(posDisparo);
    List<String> adyacentes = new ArrayList<>(P_adyacentes);
    Listas.quitaAUnaOtra(noDisparadas, adyacentes);
    List<String> puntosCardinales = Posicion.getPuntosCardinales(posDisparo);
        if(this.proximosDisparos.isEmpty()){
                for (String puntoCardinal : puntosCardinales) {
                    this.proximosDisparos.add(puntoCardinal);
                }
        }
        else{          
            if (this.tocadas.size() == 1){
                this.proximosDisparos = Posicion.getPuntosCardinales(posDisparo);
                return this.dispara();
                }
            else if (this.dirBarcoTocado == null) {
                    if (this.tocadas.get(0).charAt(0) == posDisparo.charAt(0))
                    this.dirBarcoTocado = "H";
            else
                    this.dirBarcoTocado = "V";
                }
             dejaSoloFilaOColumna(posDisparo);
        }   
        return this.dispara();
    }
